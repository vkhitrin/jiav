...
class JiraConnection(ABC):
        ...
        if not hasattr(self, "jira"):
            ...
            try:
                ...
                if "JiraError HTTP 401 url" in str(e):
                    ...
                    raise exceptions.JiraUnhandledException()
            jiav_logger.info(
                f"Successfully authenticated with Jira instance '{url}' of type '{instance_type}'"
            )
    ...
    def fetch_issues(self, issues: List[str], jql: str) -> List[Issue]:
        ...
        if issues:
            for issue in issues:
                try:
                    ...
                    #       While on-prem instances, the otuput is:
                    #       'Issue Does Not Exist'
        ...
        # Perform JQL query
            ...
            try:
                remote_issues = self.jira.search_issues(jql, json_result=False)  # type: ignore
            except JIRAError as e:
