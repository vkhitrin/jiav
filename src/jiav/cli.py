...
from jiav import logger, summary, verification, exceptions
...
@click.option(
    ...
    "--access-token",
    type=str,
    help="Personal Access Token (PAT) for self-hosted instances or an API token for cloud instances.",
    required=True,
)
@click.option(
    "-u",
...
@click.option(
    ...
    help="Uploads attachment of execution, this is not safe since all users who can access the ticket will be able to view it; refer to https://jira.atlassian.com/browse/JRASERVER-3893",
    flag_value=True,
)
@click.option(
    "--allow-public-comments",
    help="Allows to read manifest from non-private comments; this is potentially dangerous since unexpected users will be able to provide a manifest.",
    flag_value=True,
)
@click.option(
...
def verify(
    ...
    try:
        ...
        jiav_logger.exception(e)
        jiav_logger.critical(
            "Authentication with the Jira insatance failed, please check your credentials"
        )
        raise SystemExit(2)
